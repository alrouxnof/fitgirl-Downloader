...
# async def download_from_link_button(session, url, dest_path, pbar):
#     filename = url.split('/')[-1]
#     file_path = os.path.join(dest_path, filename)
...
# async def process_links_file(links_file, dest_path, site, max_concurrent=5):
#     tasks = []
#     async with aiohttp.ClientSession() as session:
#         with open(links_file, 'r') as file:
#             urls = [line.strip() for line in file if line.strip()]
...
#             for url in urls:
#                 file_pbar = tqdm(desc=f"Downloading {url.split('/')[-1]}", unit='B', unit_scale=True, position=1, leave=False)

#                 if site == 1:
#                     # Use download logic for site=1 (link-button)
#                     tasks.append(download_from_link_button(session, url, dest_path, file_pbar))
#                 else:
#                     # Generic download logic for other sites
#                     tasks.append(download_from_link_button(session, url, dest_path, file_pbar))

#                 if len(tasks) >= max_concurrent:
#                     await asyncio.gather(*tasks)
#                     tasks = []
#                     overall_pbar.update(len(tasks))
...
#     site = int(input("Enter the site (0-4, default is 1): ") or 1)
#     max_concurrent_downloads = int(input("Max number of simultaneous downloads (default: 4): ") or 4)

#     os.makedirs(download_directory, exist_ok=True)
#     asyncio.run(process_links_file(links_file, download_directory, site, max_concurrent_downloads))
